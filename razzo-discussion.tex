...
reasoning why)} \giovanni{I expect the same thing.} expected to find a bigger inference error 
...
\giovanni{Further conclusions are to be drawn after results are produced.}

\iffalse % A model can always be made more accurate. There are two principles to follow when building one: accuracy, on one hand, is surely important but, at the same time, simplicity is important too as overcomplicated models miss the point of a research question. This is, for example, summarized in the request of a low AIC, which weights positively the accuracy of the model (high likelihood) and negatively its complexity (for which the number of parameters is a proxy).
\subsection{Research}
...
\iffalse % I choose to remove this too, at least for now, as it's too soon to write any conclusion when no result is available.
...
stochasticity}. We are aware ESSes can be increased easily by
