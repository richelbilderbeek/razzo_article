...
\gio{If the environmental change triggers only $q$ is taken in consideration. $\lambda + q$ is not even a possibility, as $\lambda$ is a rate while $q$ is a probability. Do you think we need to be clearer on that?}
...
\gio{In general it is ok for me. Keep in mind, though, that allowing multiple speciations may lead to an explosion in the number of species. Increasing the time by a factor of n usually means increasing the expected number of species at the present by a factor proportional to $e^n$}.
...
Of the remaining
1000 MCMC states, the effective sample size (ESS) \gio{@richel: could it be nicer, when citing the "effective sample size" for the first time (in order to use the acronym thereafter), to put all in capital letters "Effective Sample Size"} of the posterior 
...
The input trees generated with a \richel{@gio: parameter that is set to reduce the MBD model to BD}\gio{@richel: setting $nu = 0$ denies any chance for multiple speciations to occur. If so, $q$ setting becomes unimportant. As a convention is possible to set $q=0$} 
...
\begin{table}
  ...
  \begin{tabular}{p{0.05\textwidth} p{0.6\textwidth} p{0.2\textwidth}}
    ...
    \hline
    \hline
    $\lambda$             & \gio{@richel: I insert some params that can make sense. Let's keep en eye on this anyways} & 0.1, 0.3, 1.0, $10^9$ \\
    $\lambda$             & Per-species speciation rate & 0.2 \\
    $\mu$                 & Per-species extinction rate & 0.0, 0.15 \\
    $\nu$                 & Multiple speciation trigger rate & 1.5, 2.0, 2.5 \\
    $q$                   & Per-species probability of multiple speciation  & 0.1, 0.15 \\
    \hline
    \hline
...
\richel{put this section here, as the journal does not request for this}
RSE \richel{@gio: I assume this this is true?} \gio{@richel: it is true} conceived the idea for this experiment.
